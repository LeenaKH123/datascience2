...
```{r }
numeric_data <- score_dat %>% mutate(across(where(is.character), as.numeric)) %>% mutate(across(where(is.factor), as.numeric))
numeric_data$Credit_Score <- as.numeric(score_dat$Credit_Score)
cor_matrix <- cor(numeric_data, use = "pairwise.complete.obs")
ggcorrplot(cor_matrix, hc.order = TRUE, type = "lower", lab = TRUE) + labs(title = "Correlation Heatmap")
```
```{r }
score_dat %>%
  count(Credit_Score) %>%
